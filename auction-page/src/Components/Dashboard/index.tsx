...
const Dashboard = ({players}:IProps) => {
  const renderPlayers = players.map((player:IPlayers, idx:number) => (
    <div key={idx} className={`individual_card ${player.sold_to_team_price !== 'NA' ? 'sold':''}`}>
      ...
      {player.sold_to_team_price > 0 &&  <p className="sold_price">Sold Price: {player.sold_to_team_price > 99 ? `${player.sold_to_team_price/100} Cr.`: `${player.sold_to_team_price} lakh's`}</p>}
    </div>)
  )
